...
type RequestOptions = {
  ...
  signal?: AbortSignal;
  // Opt in only when replaying the exact request is safe. The V2 upload init
  // request additionally needs its Idempotency-Key.
...
export function refreshCsrfToken(): Promise<void> {
  if (!csrfRefreshPromise) {
    csrfRefreshPromise = performCsrfRefresh().finally(() => {
...
async function performCsrfRefresh(): Promise<void> {
  let resp: Response;
  try {
    resp = await fetch(`${API_BASE_URL}/auth/csrf/refresh`, {
      method: "POST",
  ...
  const json = await parseEnvelope<never>(resp);
  if (json.code !== 0) {
...
async function request<T>(path: string, opts: RequestOptions = {}): Promise<T> {
  const { method = "GET", body, headers = {}, skipAuthRedirect = false, signal, csrfRetry } = opts;
  const url = `${API_BASE_URL}${path}`;
  const isWrite = method !== "GET" && method !== "HEAD";

  const execute = async (csrfRetried: boolean): Promise<T> => {
    ...
    const json = await parseEnvelope<T>(resp);
    if (json.code === 4036 && csrfRetry === "idempotent" && !csrfRetried) {
      try {
        await refreshCsrfToken();
      } catch (error) {
        if (error instanceof ApiError && error.code === 4010 && !skipAuthRedirect) {
          window.location.assign("/login");
        }
        throw error;
      }
