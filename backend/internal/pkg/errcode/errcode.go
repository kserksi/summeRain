...
var (
	...
	ErrRecipeVersion        = New(4261, "客户端图片配方版本不受支持", 426)
	ErrUploadSessionMissing = New(4043, "上传会话不存在或已过期", 404)
	ErrUploadConflict       = New(4091, "上传会话状态冲突", 409)
	ErrUploadIncomplete     = New(4092, "上传部件尚未全部完成", 409)
	ErrUploadBusy           = New(4291, "当前上传并发已满，请稍后重试", 429)
	ErrStoragePressure      = New(5030, "服务器存储压力过高，请稍后重试", 503)
	ErrV2UploadRequired     = New(4262, "此版本仅接受客户端预处理的 V2 上传", 426)
)
